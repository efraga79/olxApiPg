...
module.exports = {
	...
	addAction: async (req, res) => {
		...
		if(cat.length < 12) {
			res.json({error: 'ID de categorai inválido'})
			return
		}
	...
	getList: async (req, res) => {
		let { sort = 'asc', offset = 0, limit = 5, q, cat, state } = req.query
	...
	getItem: async (req, res) => {
		...
		let others = []
		if(other) {
			const otherData = await Ad.find({status: true, idUser: ad.idUser}).limit(3).exec()
	...
	editAction: async (req, res) => {
		...
		if(user._id.toString() !== ad.idUser) {
			res.json({error: 'Este anúncion não é seu'})
			return	
		}
		let updates = {}
		if(title) {
